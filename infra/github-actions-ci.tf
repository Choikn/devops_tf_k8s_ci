# ─────────────────────────────────────────────
# Github Action이 장기키(pem, access key)없이 임시 자격 증명(OIDC)을 이용 인증 역할, 권한등 처리
# 발급받아서 WEB/WAS 이미지를 ECR에 PUSH 목적 -> 보안 관리 이슈 x (깃상에 변수등이라도 존재 x)
#
# 누가 발급한 인증 토큰을 신뢰할 것인가?(발급 기관), Github에서 어떤 실행을 할때 인증 허가할 것인가?
# 이 2개에 대한 내용을 role에 반영하여 arn 구성
# ─────────────────────────────────────────────

# 현재 어떤 사용자(IAM)/역할 등이 실행중인지 => data => 조회후 채움
...
locals {
  # 위에 값을 조회하여 oidc를 발급하는 공급자의 arn 생성
  ...
  # IAM Role에서 사용가능한 Github 저장소, 브랜치 지정 (소유자, 저장소, 브랜치)
  # 2026년 7월 이후 변경된 주소 체계 때문에 이전과 같이 배치 -> 보안강화됨
...
# ─────────────────────────────────────────────
# oidc PROVIDER 생성, IAM Role을 이용하여 Githube actions를 사용하도록 신뢰 연결
# ─────────────────────────────────────────────
...
resource "aws_iam_role" "github_actions_ci" {
  ...
  # 앞서 만든 신뢰정책(인증서 발급처 신뢰, 특정소유주의 저장소/브랜치만 작동)을 본 IAM Role에 연결
...
data "aws_iam_policy_document" "github_actions_ci" {
  ...
  statement {
    ...
    actions = [
      ...
      "ecr:DescribeImages",              # 등록된 이미지, 태그 조회 (CD에서 중요)
    ...
    # 위으 권한이 허가되는 이미지를 web, was용으로 제한
    resources = [
