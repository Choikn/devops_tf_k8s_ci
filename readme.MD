...
- CI Workflow 구성
  ...
  ```
- 지금까지 구성한 인프라(aws 기반 테라폼구성)와 플랫폼/서비스(쿠버네티스) 위에 Devops 올려서 완성
...
```
        ...
        GitHub Actions
            ...
            ├─ WEB 이미지 ECR Push               # 이미지 푸시 -> 태그 수정(해시값,commit id등) -> CD 작동의 근거가 됨
            ├─ WAS 이미지 ECR Push
            └─ ECR 이미지 등록 확인
```
...
- 절차
  ...
  - 변경 : `OIDC`
    ...
    - IAM 계정별 엑세스키(개발 PC는 사용), PEM 파일 모두 사용(git 사용 x)
...
- ci 셋업 1회 진행
```
./setup-ci.bat choikn/devops_tf_k8s_ci
./setup-ci.sh choikn/devops_tf_k8s_ci
```

- 조정
  - ci.yml
    - 5, 12 라인 : 브랜치 대상 `main` 수정
